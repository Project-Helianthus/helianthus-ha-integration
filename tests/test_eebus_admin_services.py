...
class _Registry:
    ...
    def async_register(self, domain: str, name: str, handler: Any, *, supports_response: Any) -> None:
        self.registered[(domain, name)] = (handler, supports_response)
...
def test_fixed_documented_response_only_services_register_once_and_unload_after_last_entry() -> None:
    ...
    assert all(supports_response == services.SupportsResponse.ONLY for _, supports_response in hass.services.registered.values())
...
def test_fixed_service_dispatch_isolated_by_entry_and_unknown_data_rejected_before_client_call() -> None:
    ...
    handler, _ = hass.services.registered[("helianthus", FIXED_SERVICES["snapshot"])]
